...
function updateChart(measurementArray) {
  ...
  var canvas = d3.select('#chart').transition();

  x.domain(d3.extent(measurementArray, function (d) {
    var date = parsedDate(d.date);
    return date;
  }));
  y.domain([0, d3.max(measurementArray, function (d) { return d.weight; })]);
  ...
  // canvas.select('path')
  //       .duration(750)
  //       .attr('d', line(measurementArray));

  canvas.select('.area')
        .duration(750)
        .attr('d', area(measurementArray));
};
